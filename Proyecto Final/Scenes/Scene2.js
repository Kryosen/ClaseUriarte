...
class Scene2 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio("palette town", ["Sounds/palette town.mp3"]);
  }

  create() {
    this.bgMuusic = this.sound.add("palette town", {
      volume: 0.3,
      loop: false,
    });
    this.bgMuusic.play();
    ...
    const lab = map.findObject("Objects", (obj) => obj.name === "lab");

    const house = map.findObject("Objects", (obj) => obj.name === "house");

    const house2 = map.findObject("Objects", (obj) => obj.name === "house2");

    this.lab = createSpawn(lab);

    this.house = createSpawn(house);

    this.house2 = createSpawn(house2);

    // CREAMOS UNA CAJA ALREDEDOR DEL PERSONAJE PARA QUE PUEDA INTERACTUAR CON LAS FISICAS DEL JUEGO
    player = this.physics.add
      .sprite(spawnPoint.x + 8, spawnPoint.y + 8, "atlas", "trainer-front")
      .setSize(14, 14)
      .setOffset(1, 1);
